...
from flask import Flask, render_template, flash, request, redirect, url_for, session, jsonify, abort, make_response
...
def uploadpreview():
    if request.method == 'POST':
        ...
        storagePath = "sakelabels/" + img_filename
        ...
        db.collection(u'labels').document().set({
            ...
            u'filename': img_filename,
            u'storageurl': storageurl,
            u'description': pureText,
        })
...
class AddSakeForm(Form):
    maker = SelectField('Maker:', choices=[], validators=[
                        validators.required()])
...
def add_sake_to_db(maker, name, saketype, taste, description):
    makers_ref = db.collection(u'makers')
    docs = makers_ref.get()
    for doc in docs:
...
def addsake():
    ...
    if request.method == 'POST':
        ...
        if form.validate():
            add_sake_to_db(maker, name, saketype, taste, description)
